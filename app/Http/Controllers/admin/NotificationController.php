...
class notificationController extends Controller
{
 ...
 public function announce_detail(Request $request){
    ...
    public function save_rankType(Request $request){
        ...
        //$checkRange="select id from rank_types where 8 between range_from and range_to" ;
...
public function sendNotificationToAllUser(Request $request){
     $announcementId = $request->nid ;
     $announceInfo= DB::table('pe_announcement_list')->select('title','content')->where('status',1)->where('id',$announcementId)->first();
     $title=isset($announceInfo->title)?$announceInfo->title:'' ;
     $message=isset($announceInfo->content)?$announceInfo->content:'' ;
   
    $d="INSERT INTO `notifications` (`userId`,`nId`,`hostId`, `title`, `message`, `data`, `isAccept`, `isSend`, `isRead`, `status`) select userId,$announcementId, '0', '".$title."' , '".$message."', '', '0', '1', '0', '1' from device_token ";
               
    DB::insert($d);

     $deviceToken = DB::table('device_token')->where('deviceToken','<>','')->pluck('deviceToken')->toArray();
     $this->sendPushNotification($deviceToken,$title,$message);
    
}

function sendPushNotification($device_token,$title,$body){  
