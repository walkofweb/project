...
public function createThumbnail($path, $width, $height)
    ...
    public function sponserList(){
     ...
     public function sponser_datatable(Request $request){
        ...
        $carQry="select s.id,s.name,s.email,case when (s.image is null || s.image='') then '' else concat('".$img."',s.image) end as image,s.description,case when s.createdBy=0 then 'Admin' when u.name is null then '' else u.name end as createdBy,case when s.status=1 then 'Active' else 'Inactive' end as status_,s.status as status from sponser as s left join users as u on u.id=s.createdBy" ;  
        $carData = DB::select($carQry); 
        $tableData = Datatables::of($carData)->make(true);  
        return $tableData; 
    }
